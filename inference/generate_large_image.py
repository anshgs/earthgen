...
parser.add_argument("-lr", "--lowres",help="low-res image path", required=True)
...
def crop_center(im, dim1, dim2):
    width, height = im.size
    left = (width - dim2)/2
    top = (height - dim1)/2
    right = (width + dim2)/2
    bottom = (height + dim1)/2
    ret = im.crop((left, top, right, bottom))
    return ret

def crop_center2(im, dim1, dim2):
    left = 950
    top = 970
    right = left+dim2
    bottom = top+dim1
...
def get_image_mixture(lrim, unet, bs, timesteps_k, latent_save_path, guidance_scale):
    unet.eval()
    latents = torch.randn((1, 4, out_dim[0]//4, out_dim[1]//4)).to('cuda')
    prompt_embeds = text_encoder(torch.unsqueeze(get_tokenized_caption("satellite photo, fall colors, autumn leaves")[0], dim=0).to('cuda'))[0]
    ...
    views = get_views(out_dim[0], out_dim[1])
        ...
        for i, t in enumerate(tqdm(timesteps,desc="timesteps", position=0,leave=True)):
            ...
            for j, view in enumerate(tqdm(views, desc="views", position=1,leave=True)):
                ...
                if curcount == bs or j == len(views)-1:
                    ...
                    noise_pred = unet(torch.cat(stacked), torch.tensor([t]*curcount), prompt_embeds_batched, class_labels=torch.tensor([20]*curcount, dtype=torch.long, device='cuda'), return_dict=False)[0].cpu()
                    noise_pred_neg = unet(torch.cat(stacked), torch.tensor([t]*curcount), neg_embeds_batched, class_labels=torch.tensor([20]*curcount, dtype=torch.long, device='cuda'), return_dict=False)[0].cpu()
...
def generate_next_zoom(low_res, unet, batch_size, timesteps, latent_save_path, image_save_path, guidance_scale):
    ...
    final_im = get_out_image(latent_save_path, image_save_path, batch_size)

import os
import shutil


cat_dir_new = args.outdir
if not os.path.exists(cat_dir_new):
    os.makedirs(cat_dir_new)
Image.MAX_IMAGE_PIXELS = 933120000

low_res_path = args.lowres
low_res = Image.open(low_res_path)
low_res = crop_center2(low_res, 128, 192)
low_res.save(os.path.join(cat_dir_new, "10_gt.png"))
out_dim = (512,768)
unet = torch.nn.DataParallel(UNet2DConditionModel.from_pretrained(
            os.path.join(args.load, '10to12long'), subfolder="unet"
        ), device_ids=[0,1,2,3], dim=0).cuda()
path_12 = os.path.join(cat_dir_new, "10_gt_12.png")
generate_next_zoom(low_res, unet, batch_size, 50, '/scratch/bbut/axiao/latents_large_10.pt', path_12, 5)

low_res = Image.open(path_12)
low_res = crop_center(low_res, 256, 768)
out_dim = (1024,3072)
unet = torch.nn.DataParallel(UNet2DConditionModel.from_pretrained(
            os.path.join(args.load, '12to14long'), subfolder="unet"
        ), device_ids=[0,1,2,3], dim=0).cuda()
path_14 = os.path.join(cat_dir_new, "10_gt_14.png")
generate_next_zoom(low_res, unet, batch_size, 50, '/scratch/bbut/axiao/latents_large_12.pt', path_14, 2)
...
unet = torch.nn.DataParallel(UNet2DConditionModel.from_pretrained(
            os.path.join(args.load, '14to16long'), subfolder="unet"
        ), device_ids=[0,1,2,3], dim=0).cuda()
path_16 = os.path.join(cat_dir_new, "10_gt_16.png")
generate_next_zoom(low_res, unet, batch_size, 50, '/scratch/bbut/axiao/latents_large_14.pt', path_16, 5)

low_res = Image.open(path_16)
out_dim = (16384,49152)
batch_size=128
unet = torch.nn.DataParallel(UNet2DConditionModel.from_pretrained(
            os.path.join(args.load, '16to18long'), subfolder="unet"
        ), device_ids=[0,1,2,3], dim=0).cuda()
path_18 = os.path.join(cat_dir_new, "10_gt_18.png")
generate_next_zoom(low_res, unet, batch_size, 50, 'latents_large_16.pt', path_18, 3)


low_res = Image.open(path_18)
out_dim = (65536, 196608)
batch_size=128
unet = torch.nn.DataParallel(UNet2DConditionModel.from_pretrained(
            os.path.join(args.load, '18to20long'), subfolder="unet"
        ), device_ids=[0,1,2,3], dim=0).cuda()
path_20 = os.path.join(cat_dir_new, "10_gt_20.png")
generate_next_zoom(low_res, unet, batch_size, 50, 'latents_large_18.pt', path_20, 4)
